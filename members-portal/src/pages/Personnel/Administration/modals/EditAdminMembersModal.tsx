...
interface EditAdminMembersFormData {
    newTeamId: string;
    newRoleId: string;
    newSubteamId: string;
...
interface EditAdminMembersErrors {
    newTeamId?: string;
    newRoleId?: string;
    newSubteamId?: string;
    leaveType?: string;
...
interface EditAdminMembersSubmitPayload {
    member: ModalMember;
    changes: {
        editMode: EditMode;
        formData: EditAdminMembersFormData;
    };
}

interface EditAdminMembersModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSubmit?: (payload: EditAdminMembersSubmitPayload) => Promise<void> | void;
...
const EditAdminMembersModal = ({
    ...
    subteams = [],
    occupiedRoleIds = [],
}: EditAdminMembersModalProps) => {
    void occupiedRoleIds;

    const [editMode, setEditMode] = useState<EditMode>(EDIT_MODES.PROMOTION_DEMOTION);
    const [formData, setFormData] = useState<EditAdminMembersFormData>({
        newTeamId: '',
        newRoleId: '',
        newSubteamId: '',
    ...
    const [errors, setErrors] = useState<EditAdminMembersErrors>({});
    const [isSubmitting, setIsSubmitting] = useState(false);
    const [successMessage, setSuccessMessage] = useState('');
    const [availableRoles, setAvailableRoles] = useState<ModalRole[]>([]);
    const [, setAvailableSubteams] = useState<ModalSubteam[]>([]);
    ...
    useEffect(() => {
        if (isOpen && member && currentTeamAssignment) {
            setEditMode(EDIT_MODES.PROMOTION_DEMOTION);
            setFormData({
                newTeamId: '',
    ...
    const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setFormData((prev) => ({
            ...prev,
            [name]: value,
        } as EditAdminMembersFormData));

        const key = name as keyof EditAdminMembersErrors;
        if (errors[key]) {
    ...
    const setMode = (mode: EditMode) => {
        ...
        } else if (mode === EDIT_MODES.PROMOTION_DEMOTION) {
            setFormData((prev) => ({
                ...prev,
                newRoleId: currentTeamAssignment?.roleId != null ? String(currentTeamAssignment.roleId) : '',
                newSubteamId: '',
    ...
    const validate = (): boolean => {
        const newErrors: EditAdminMembersErrors = {};
                        ...
                        <div className="form-section">
                            ...
                            <div className="radio-group-list">
                                <label className={`radio-option-card ${editMode === EDIT_MODES.PROMOTION_DEMOTION ? 'selected' : ''}`}>
                                    ...
                                    />
                                    <span className="radio-option-title">1. Promotion / Demotion</span>
                                    <span className="radio-option-desc">Promote or demote the member; optionally change role within Administration</span>
                                </label>
                                ...
                                <label className={`radio-option-card ${editMode === EDIT_MODES.TRANSFER ? 'selected' : ''}`}>
                                    <input
                                        ...
                                        disabled={isSubmitting}
                                    />
                                    <span className="radio-option-title">2. Transfer</span>
                                ...
                                <label className={`radio-option-card ${editMode === EDIT_MODES.LEAVE ? 'selected' : ''}`}>
                                    <input
                                        ...
                                        disabled={isSubmitting}
                                    />
                                    <span className="radio-option-title">3. Graduate / Expulsion / Resignation</span>
                            ...
                            <div className="form-section conditional-section">
                                ...
                                <div className="form-group">
                                    ...
                                    >
                                        ...
                                        {availableRoles
                                            .filter((role) => role.roleName !== 'Officer')
                                            .map((role) => {
                                                const isOwnRole = role.id === currentTeamAssignment?.roleId;
                                                const count = teamMembersForTeam.filter(
...
export default EditAdminMembersModal;
