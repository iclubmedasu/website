...
function getExportContentWidth(values = [], { minWidth = 8, maxWidth = 16, padding = 2 } = {}) {
...
function formatExportShortDate(value) {
    const date = getDateOrNull(value);
    if (!date) return '';
...
function getExportRowRange(row) {
...
function buildExportRows(phases = []) {
    const exportRows = [];

    const pushRow = (type, data, depth, phase = null, parentTask = null) => {
        ...
        exportRows.push({
            ...
            dependencies: dependencies
                .map((dependency) => dependency.dependsOnTask?.title || dependency.dependsOnTask?.fileName || dependency.dependsOnTaskId)
                .filter(Boolean)
                .join(', '),
            assignees: assignments
                .map((assignment) => assignment.member?.fullName || assignment.fullName || assignment.name || assignment.memberName)
                .filter(Boolean)
                .join(', '),
...
function getExportDateRange(phases = [], projectStartDate = null, projectDueDate = null) {
...
function buildExportTimeline(rangeStart, rangeEnd) {
    const days = [];
    ...
    const buildBands = (keyFn, labelFn) => {
        const bands = [];
        let current = null;
...
function collectScheduleTimelineSlots(projectDetail, allTaskNodes = []) {
    const slotSources = [
...
function formatScheduleClockRange(start, end) {
...
function getScheduleMemberSortLabel(slot) {
    const fullName = String(slot?.member?.fullName || '').trim();
    if (fullName) return fullName;
...
function getScheduleSlotLabel(slot) {
    return String(slot?.title || slot?.task?.title || 'Time slot').trim();
}

function buildScheduleTimelineExport(slots = []) {
    const visibleDays = [];
...
function groupScheduleSlotsByMember(slots = []) {
...
function buildScheduleTimelineSheet(XLSX, slots = [], title = 'Schedule Timeline') {
    ...
    const styleCell = (rowIndex, colIndex, styles, value = '') => {
...
function applySheetCellStyle(cell, {
    fill,
    fontColor = '000000',
    bold = false,
    italic = false,
...
export default function GanttChart({
    ...
    const openInlineEditor = useCallback((row, col) => {
        flushSync(() => {
    ...
    const exportAsPng = useCallback(async () => {
        ...
        try {
            ...
            const allPhaseIds = new Set(phases.map((phase) => phase.id));
