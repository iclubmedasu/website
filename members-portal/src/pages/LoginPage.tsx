...
function isValidEmailFormat(value: string): boolean {
    return !value || EMAIL_FORMAT.test(value.trim());
}
...
function getProgressIndex(step: LoginStep): number {
    switch (step) {
        case 'studentId':
            return 0;
        case 'setupProfile':
...
function showProgressBar(step: LoginStep): boolean {
...
function LoginPage() {
    ...
    const [identifierMode, setIdentifierMode] = useState<IdentifierMode>('text');
    ...
    const handleSetupProfileSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setError('');
    ...
    const handleStudentIdSubmit = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setError('');
        setLoading(true);
        ...
        if (result.success && result.data.canSetup) {
            ...
            const fallback =
                result.data?.message ||
                'Cannot set up with this Student ID. Use "Enter your email" if you already have an account.';
            setError(result.success ? fallback : result.error || fallback);
        }
    };

    const handleCompleteProfileDetailsContinue = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setError('');
    ...
    const handleOfficerProfileDetailsContinue = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        setError('');
