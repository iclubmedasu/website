...
export interface AnnouncementResponseExportRow {
    status: string;
    notes: string | null;
...
function sanitizeFileName(name: string): string {
    ...
    return cleaned || 'announcement-responses';
}
...
function buildResponsesMatrix(rows: AnnouncementResponseExportRow[]): string[][] {
    return [
        ['Member', 'Status', 'Availability', 'Notes', 'Submitted', 'Updated'],
        ...rows.map((row) => [
            row.member.fullName,
            statusLabel(row.status),
            row.periods.length > 0 ? formatPeriods(row.periods) : '',
            row.notes?.trim() ?? '',
...
export async function exportAnnouncementResponsesExcel(
    title: string,
    rows: AnnouncementResponseExportRow[],
): Promise<void> {
    ...
    downloadBlob(workbookBlob, `${sanitizeFileName(title)}-responses.xlsx`);
}
