...
export default function FileUploadZone({ projectId, memberId, onFileUploaded, onFileRemoved, onFileRenamed, existingFiles = [], existingFolders = [], disabled }) {
    ...
    const [folderAction, setFolderAction] = useState(null); // { type: 'delete' | 'restore' | 'history', folder }
    const [folderActionLoading, setFolderActionLoading] = useState(false);
    const [folderActionError, setFolderActionError] = useState('');
    const [folderHistoryData, setFolderHistoryData] = useState([]);
    ...
    const handleFileDragStart = useCallback((fileEntry, event) => {
    ...
    const moveFileToFolder = useCallback(async (fileEntry, folderId) => {
        ...
        try {
            const moved = await projectFilesAPI.move(fileEntry.id, folderId);
    ...
    const handleRepositoryDragOver = useCallback((event) => {
    ...
    const handleRepositoryDrop = useCallback((event) => {
        if (disabled || draggingFileId == null) return;
        event.preventDefault();
        event.stopPropagation();
    ...
    const processFiles = useCallback((fileList, folderIdOverride = null) => {
        if (disabled) return;
    ...
    const refreshFoldersAfterAction = useCallback((updatedFolder) => {
        setFolders((prev) => prev.map((folder) => (folder.id === updatedFolder.id ? updatedFolder : folder)));
    }, []);

    const openFolderHistory = async (folder) => {
    ...
    const openFolderAction = (type, folder) => {
    ...
    const confirmFolderAction = async () => {
        ...
        try {
            if (type === 'delete') {
                const result = await projectFilesAPI.removeFolder(folder.id);
                refreshFoldersAfterAction(result.folder);
            } else if (type === 'restore') {
                const result = await projectFilesAPI.restoreFolder(folder.id);
                refreshFoldersAfterAction(result);
            }
            closeFolderAction();
        } catch (err) {
            console.error('Folder action failed:', err);
            setFolderActionError(err.message || 'Failed to update folder');
            setFolderActionLoading(false);
        }
    };
    ...
    const confirmCreateFolder = async () => {
        ...
        setFolderCreateError('');
        try {
            const created = await projectFilesAPI.createFolder(projectId, trimmed, memberId);
    ...
    const renderFileItem = (f, { nested = false } = {}) => {
                                            ...
                                            {deletedFiles.map((f) => (
                                                <div key={f.id} className="file-item file-item--deleted">
                                                    ...
                                                    <div className="file-info">
                                                        ...
                                                        <span className="deleted-file-date">Deleted {formatHistoryDate(f.updatedAt)}</span>
                                                    </div>
                                                    <div className="file-actions">
