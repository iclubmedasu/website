...
export default function FileCommentsModal({ file, filesAPI, onClose }: FileCommentsModalProps) {
    const [comments, setComments] = useState<EntityFileCommentRef[]>([]);
    ...
    const loadComments = async () => {
        ...
        try {
            setLoading(true);
            setError('');
            const data = await filesAPI.getComments(file.id);
    ...
    const handleAddComment = async () => {
        ...
        try {
            setSaving(true);
            setError('');
            await filesAPI.addComment(file.id, commentText.trim());
            setCommentText('');
            await loadComments();
        } catch (err: unknown) {
            setError(getErrorMessage(err));
