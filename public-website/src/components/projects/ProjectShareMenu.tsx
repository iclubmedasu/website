...
import "@/components/events/event-share.css";

interface ProjectShareMenuProps {
    projectSlug: string;
    projectTitle: string;
    className?: string;
}
...
export function ProjectShareMenu({
    ...
    const url = buildPublicProjectUrl(projectSlug);
    const message = buildShareMessage(projectTitle, url);
    ...
    const handleNativeShare = async (event: React.MouseEvent<HTMLButtonElement>) => {
        stopPropagation(event);
        const ok = await shareViaNative(projectTitle, url);
        if (ok) {
            closeMenu();
        }
    };
            ...
            {open ? (
                <div id={menuId} className="event-share-menu" role="menu">
                    ...
                    <a
                        role="menuitem"
                        className="event-share-menu-item"
                        href={buildEmailShareUrl(projectTitle, message)}
                        onClick={handleExternalLink}
                    >
