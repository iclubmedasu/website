...
function headers(extraAccept = "application/vnd.github+json"): Record<string, string> {
    return {
...
async function uploadFile(
    ...
    const githubPath = replaceOpts.existingPath || `projects/${projectId}/${uniqueName}`;
...
async function moveFile(
    fromGithubPath: string,
    toGithubPath: string,
    fromGithubSha: string,
    message: string,
): Promise<UploadResult> {
    ...
    if (!newSha) {
        throw new Error('GitHub move upload succeeded but SHA was missing from response');
    }
 ...
 */
async function deleteFile(githubPath: string, githubSha: string): Promise<void> {
 ...
 * @returns {Response}
 */
async function downloadFile(githubPath: string): Promise<Response> {
    const res = await fetch(`${BASE}/${githubPath}`, {
        headers: headers('application/vnd.github.raw+json'),
    });
 ...
 */
async function getFileHistory(githubPath: string): Promise<GithubHistoryEntry[]> {
 ...
 * @returns {Response}
 */
async function downloadFileAtVersion(githubPath: string, commitSha: string): Promise<Response> {
 ...
 */
async function restoreDeletedFile(githubPath: string): Promise<{ githubSha: string }> {
