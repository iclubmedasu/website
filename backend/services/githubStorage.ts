...
const ALL_EXTS: PhotoExt[] = ['.jpg', '.png', '.webp'];
 ...
 */
async function findExistingPhoto(memberId: number | string): Promise<ExistingPhoto | null> {
 ...
 */
async function uploadProfilePhoto(memberId: number, fileBuffer: Buffer, mimeType: string): Promise<string> {
 ...
 */
async function deleteProfilePhoto(memberId: number): Promise<void> {
    const existing = await findExistingPhoto(memberId);
...
const _photoCache = new Map<number | string, CachedPhoto>();
 ...
 */
async function downloadProfilePhoto(memberId: number): Promise<{ buffer: Buffer; contentType: string } | null> {
 ...
 * @param {number} memberId
 */
function invalidatePhotoCache(memberId: number): void {
    _photoCache.delete(memberId);
}

export { uploadProfilePhoto, deleteProfilePhoto, downloadProfilePhoto, invalidatePhotoCache };
